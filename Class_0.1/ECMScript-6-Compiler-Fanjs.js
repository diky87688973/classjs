 ...
 * 
 * 模拟 ECMAScript 6 草案规定的类系统, 使得在不支
 * 持 ECMAScript 6 的浏览器上, 也能正常使用类机制
 * 
 * 将 ECMAScript 6 代码转换成新的 Fanjs 所支持的语法
 * 
 * 示例:
 * 
 * 定义类:
 ...
 * 定义接口:
 * interface IMyItface {
 *  method1();  // 简洁写法的接口方法定义, 效果同等于完整写法: this.method1 = Function;
 *  method2();
 ...
 *
 *  var a = 'aaa';          // 私有成员
 *  this.name = 'dog';      // 公有成员, 实例通过 obj.b 访问
 *
 *  Dog( age, sex, name ) {
 ...
 *  }
 *
 *  say() { // 简洁写法的成员方法定义, 效果同等于完整写法: this.say = function(...){}
 *      return this.name + ',' + super(); // super() 调用父类同名方法
 *  }
 * }
 * 
 * 实现接口
 ...
 *
 *  method1() {
 *      return 'method1 call ' + this.property1;
 *  }
 *  
 *  method2() {
 *      return 'method2 call ' + this.property2;
 *  }
 * }
 * 
 * 继承类同时实现接口
 ...
 * 编译器基类
 */
Fan.Class( 'Compiler', function () {
    /* [system embedded code] */
    var Super = null, This = null;
    this.Compiler = this.Compiler || function() {
        Super();
    };
    this[ Fan.KEYS.SET_SUPER ] = function( s ) {
        Super = s;
        This = s ? s[ Fan.KEYS.THIS ] : null;
    };
    this[ Fan.KEYS.GET_SUPER ] = function( b ) {
        return b ? Super[ Fan.KEYS.SUPER ] : Super;
    };
    /* [system embedded code] */
     ...
     * @param buildErrorCallback        - 编译错误时的回调, 接受数组参数:[错误行,错误列,错误描述,错误代码]
...
/**
 * ECMAScript 6 OOP 编译器
 * 
 * 转化ECMAScript代码为Fanjs的形式, 用于跨浏览器支持 ECMAScript 6 OOP 语法
 * 
 * 尚未实现:
 * 1, 成员方法收集, 调用允许缺省前缀 this.
 * 2, 成员属性收集, 访问成员属性时, 允许缺省前缀 this.
 * 
 */
Fan.Class( 'JavascriptCompiler', Compiler, function () {
    ...
    this.processKeyword = function( keyword, code, index ) {
        switch ( keyword ) {
        case 'package' :
            code = this.insertCodeAtIndex( 'Fan.Package(', code, index, keyword.length );
        ...
        case 'class' :
            ...
            inClass = true;
            code = this.insertCodeAtIndex( 'Fan.Class(', code, index, keyword.length );
        ...
        case 'interface' :
            _hasClassKeyword = true;
            inInterface = true;
            code = this.insertCodeAtIndex( 'Fan.Interface(', code, index, keyword.length );
            // [重要回调设置]:根据当前关键字, 预设最临近的一个标识符.
            // 如:当前是class关键字,下一个标识符为类名, 故而需要替换前后带引号并且增加前缀包名
            nextIdentifier = function( identifier ) {
                nextIdentifier = null;
        ...
        case 'import' : // 暂不支持一个import导入多个文件的写法
            code = this.insertCodeAtIndex( 'Fan.Import(', code, index, keyword.length );
            nextIdentifier = function( identifier ) {
                nextIdentifier = null;
                return '"' + identifier + '" )';
            };
    ...
    this.processItfsFunction = function( funcName, code, startIndex, curIndex ) {
        code = this.insertCodeAtIndex( 'void(0', code, curIndex, 1 ); // ( -> void(0
    ...
    this.insertCodeAtIndex = function( newCode, srcCode, index, overrideLength ) {
        overrideLength >>= 0;
        console.log( '%c ' + srcCode.substring( index, index + overrideLength ) + '%c ->%c ' + newCode, 'color:#777', 'color:#ccc', 'color:#333' );
        return srcCode.substring( 0, index ) + newCode + srcCode.substring( index + overrideLength );
    };
    
    // 检索是否使用了Fan.Class()方法所用的正则
    var _checkClassReg = /((?:^|[,;\(\){}=]|(?:\*[\/])|(?:\/\/[^\n]*\n))\s*(?:Fan|OOP)\s*\.\s*(Class|Interface)(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*\((?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*(['"])([a-zA-Z0-9._$]+)\3(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*(?:,(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*[a-zA-Z0-9._$]+(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*)*?,(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*function(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*\((?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*\)(?:\s|\/\*[\s\S]*?\*\/|\/\/[^\n]*\n)*{)/;
    var _keywordReg = /^(?:package|import|class|interface|extends|implements|new|this|super|var|let|const|function|do|while|for|in|continue|break|switch|case|default|return|throw|try|catch|finally|delete)$/;
    ...
    this.build = function( code, buildSuccessCallback, buildErrorCallback, buildStepByCodeCallback ) {
            ...
            if ( 0 === sinYH.length && 0 === dblYH.length && 0 === regKH.length ) {
                ...
                if ( flg && keyword.length ) {
                    ...
                    if ( _keywordReg.test( keyword ) ) {
                        // console.log( '%c 关键字:' + keyword, 'color:blue' );
                        // 关键字处理
                        var tmpCode = this.processKeyword( keyword, code, keywordIndex );
                    ...
                    // 检测方法标识符
                        ...
                        if ( /^[a-z0-9_$\s]+\($/i.test( keyword ) ) {
                            if ( inClass ) {
                                console.log( '%c 成员方法:' + keyword, 'color:#8A2BE2' );
                                
                                // 处理成员方法
                                var tmpCode = this.processFunction( keyword.substring( 0, keyword.length - 1 ).trim(), code, keywordIndex );
                                i += tmpCode.length - code.length;
                                code = tmpCode;
                            } else if ( inInterface ) {
                                console.log( '%c 接口方法:' + keyword, 'color:red' );
                                
                                // 处理接口方法
                                //var tmpCode = this.processItfsFunction( keyword.substring( 0, keyword.length - 1 ).trim(), code, keywordIndex );
                                var tmpCode = this.processItfsFunction( keyword.substring( 0, keyword.length - 1 ).trim(), code, keywordIndex, i );
                                i += tmpCode.length - code.length;
                                code = tmpCode;
                            }
                        }
                    ...
                    // 纯标识符
                        ...
                        // console.log( '%c 标识符:' + keyword, 'color:green' );
                        
                        // 标识符处理
                        var tmpCode = this.processIdentifier( keyword, prevKeyword, code, keywordIndex );
                        i += tmpCode.length - code.length;
                        code = tmpCode;
                        prevKeyword = '';
                        keyword = '';
        ...
        if ( errIdx >= 0 ) {
            // 截取错误断点之前的代码
            var tmpCode     = code.substring( 0, errIdx ) || '',
    ...
    this.coloring = function( code ) {
        ...
        // 关键字
        code = code.replace(
                /\b(package|import|class|interface|extends|implements|new|this|super|var|let|const|function|do|while|for|in|continue|break|switch|case|default|return|throw|try|catch|finally|delete)\b/g,
                function( v1, v2, v3 ) {
    ...
    /* [system embedded code] */
    var Super = null, This = null;
    this.JavascriptCompiler = function() {
        Super();
    };
    this[ Fan.KEYS.SET_SUPER ] = function( s ) {
        Super = s;
...
//      
//      // 创建javascript编译器
//      window.compiler = new JavascriptCompiler( 'aaa' );
//      
